program test_pty_lifecycle
  use fgof_pty, only : FGOF_PTY_ERR_IO_FAILED, FGOF_PTY_OK, close_pty, pty_session, read_some, spawn_pty, wait_pty, write_all
  implicit none
  ...
  if (.not. wait_pty(session, 3000)) error stop "wait_pty should observe child exit"
  ...
  output = read_some(session, 512)
  if (output /= "") error stop "EOF read should return empty text"
  if (.not. session%eof_reached) error stop "EOF should be recorded after child exit"

  if (write_all(session, "should fail")) error stop "write_all should fail after child exit"
  if (session%error_code /= FGOF_PTY_ERR_IO_FAILED) error stop "write-after-exit should report I/O failure"
